...
export class ComputerWriteController {
     ...
     * gesetzt und genauso auch wenn der Name oder die Artikelnummer bereits
     * existieren.
     *
    ...
    @ApiOperation({ summary: 'Ein neuen Computer anlegen' })
     ...
     * Name oder die neue Artikelnummer bereits existieren.
     *
    ...
    @Roles('admin', 'mitarbeiter')
    @ApiOperation({
        summary: 'Ein vorhandenen Computer aktualisieren',
        tags: ['Aktualisieren'],
    })
    @ApiHeader({
        name: 'If-Match',
    ...
    #handleCreateError(err: CreateError, res: Response) {
        switch (err.type) {
            ...
            case 'ArtikelnummerExists': {
                return this.#handleArtikelnummerExists(err.artikelnummer, res);
            }
    ...
    #handleArtikelnummerExists(
        artikelnummer: number | null | undefined,
        res: Response,
    ): Response {
        const msg = `Die Artikelnummer "${artikelnummer}" existiert bereits.`;
        this.#logger.debug('#handleArtikelnummerExists(): msg=%s', msg);
        return res
            .status(HttpStatus.UNPROCESSABLE_ENTITY)
            .set('Content-Type', 'text/plain')
            .send(msg);
