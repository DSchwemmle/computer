...
/**
 * Die Klasse `QueryBuilder` implementiert das Lesen für ComputerList und greift
...
export class QueryBuilder {
    ...
    build(suchkriterien: Record<string, any>) {
        ...
        // z.B. { name: 'MacBook Air', artikelnummer: 12345678, lieferbar: true }
        // Rest Properties fuer anfaengliche WHERE-Klausel
        // type-coverage:ignore-next-line
        const { name, artikelnummer, ...props } = suchkriterien;

        let useWhere = true;

        // Namen in der Query: Teilstring des Namens und "case insensitive"
        ...
        if (name !== undefined && typeof name === 'string') {
            ...
            queryBuilder = queryBuilder.where(
                `${this.#computerAlias}.name ${ilike} :name`,
                { name: `%${name}%` },
            );
            useWhere = false;
        }
        ...
        // type-coverage:ignore-next-line
        if (artikelnummer !== undefined && typeof artikelnummer === 'number') {
            const param = {
                artikelnummer,
            };
            queryBuilder = useWhere
                ? queryBuilder.where(
                      `${this.#computerAlias}.artikelnummer = :artikelnummer`,
                      param,
                  )
